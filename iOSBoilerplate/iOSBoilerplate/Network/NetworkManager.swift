...
class NetworkManager {
    ...
    func request<T:Decodable>(_ api: APIEndpoints, type: T.Type) async throws -> T {
        ...
        do {
            ...
            AnalyticsManager.shared.log(event: Event.error(name: "Decoding error (\(api.data.0), \(api.data.1))", error: error))
        ...
        try await withCheckedThrowingContinuation { continuation in
            request(api) { result in
                if case .failure(let error) = result {
                    AnalyticsManager.shared.log(event: Event.error(name: "Request failed (\(api.data.0), \(api.data.1))", error: error))
                }
